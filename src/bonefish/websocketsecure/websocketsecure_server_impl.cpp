...
#include <bonefish/websocketsecure/websocketsecure_protocol.hpp>
...
websocketsecure_server_impl::websocketsecure_server_impl(
        boost::asio::io_service& io_service,
        const std::shared_ptr<wamp_routers>& routers,
        const std::shared_ptr<wamp_serializers>& serializers)
    : m_io_service(io_service)
    , m_server(new websocketpp::server<websocketsecure_config>())
    , m_routers(routers)
...
void websocketsecure_server_impl::start(const boost::asio::ip::address& ip_address, uint16_t port)
{
    BONEFISH_TRACE("starting websocket server: %1%:%2%", ip_address.to_string() % port);

    boost::asio::ip::tcp::endpoint endpoint(ip_address, port);

    std::weak_ptr<websocketsecure_server_impl> weak_self = shared_from_this();
    ...
    auto message_handler = [weak_self](
            websocketpp::connection_hdl handle,
            websocketpp::server<websocketsecure_config>::message_ptr message) {
        auto shared_self = weak_self.lock();
        if (shared_self) {
...
void websocketsecure_server_impl::shutdown()
{
    BONEFISH_TRACE("stopping websocket server");
    m_server->stop_listening();
}

void websocketsecure_server_impl::on_socket_init(websocketpp::connection_hdl handle,
        boost::asio::ip::tcp::socket& s)
{
    s.set_option(boost::asio::ip::tcp::no_delay(true));
}

void websocketsecure_server_impl::on_open(websocketpp::connection_hdl handle)
{
}

void websocketsecure_server_impl::on_close(websocketpp::connection_hdl handle)
{
    websocketpp::server<websocketsecure_config>::connection_ptr connection =
...
void websocketsecure_server_impl::on_fail(websocketpp::connection_hdl handle)
{
    websocketpp::server<websocketsecure_config>::connection_ptr connection =
            m_server->get_con_from_hdl(handle);
...
bool websocketsecure_server_impl::on_validate(websocketpp::connection_hdl handle)
{
    websocketpp::server<websocketsecure_config>::connection_ptr connection =
            m_server->get_con_from_hdl(handle);
...
void websocketsecure_server_impl::on_message(websocketpp::connection_hdl handle,
        websocketpp::server<websocketsecure_config>::message_ptr buffer)
{
    websocketpp::server<websocketsecure_config>::connection_ptr connection =
            m_server->get_con_from_hdl(handle);
    ...
    try {
        ...
        std::unique_ptr<wamp_transport> transport(
                new websocketsecure_transport(serializer, handle, m_server));
