...
#ifndef BONEFISH_WEBSOCKETSECURE_WEBSOCKET_SERVER_IMPL_HPP
#define BONEFISH_WEBSOCKETSECURE_WEBSOCKET_SERVER_IMPL_HPP
...
class websocketsecure_server_impl : public std::enable_shared_from_this<websocketsecure_server_impl>
{
public:
    websocketsecure_server_impl(boost::asio::io_service& io_service,
            const std::shared_ptr<wamp_routers>& routers,
            const std::shared_ptr<wamp_serializers>& serializers);
    ~websocketsecure_server_impl();
...
#endif // BONEFISH_WEBSOCKETSECURE_WEBSOCKET_SERVER_IMPL_HPP
